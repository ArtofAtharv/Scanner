...
import { Users, Utensils, Coffee, Moon, Edit2, Trash2, Check, X } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const highTeaCount = participants.reduce(
    (acc, p) => acc + (p.meal_usage?.find((m) => m.meal_type === "high_tea")?.is_used ? 1 : 0),
    0
  );
  const lunchCount = participants.reduce(
    (acc, p) => acc + (p.meal_usage?.find((m) => m.meal_type === "lunch")?.is_used ? 1 : 0),
    0
  );
  const dinnerCount = participants.reduce(
    (acc, p) => acc + (p.meal_usage?.find((m) => m.meal_type === "dinner")?.is_used ? 1 : 0),
    0
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          {/* Add Form */}
          <section className="lg:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 h-fit">
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full px-4 py-2 rounded-md border border-gray-300 dark:border-gray-600 focus:ring-indigo-500 dark:bg-gray-900 dark:text-white"
                />
              </div>
              <div>
              ...
              >
                {adding ? "Adding..." : "Add & Send QR"}
              </button>
            </form>
          </section>

          {/* Table */}
          <section className="lg:col-span-3 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-gray-50 dark:bg-gray-900 text-gray-600 dark:text-gray-400">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-medium cursor-help" title="High Tea">☕ HT</th>
                    <th className="px-4 py-3 font-medium cursor-help" title="Lunch">🍱 LU</th>
                    <th className="px-4 py-3 font-medium cursor-help" title="Dinner">🌙 DI</th>
                    ...
                    participants.map((p) => {
                          ...
                          <tr key={p.id} className="bg-indigo-50/50 dark:bg-indigo-900/20">
                            ...
                            <td className="px-6 py-4">
                              <select
                                ...
                                className="px-2 py-1 text-sm rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                              >
                                {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                              </select>
                            </td>
                            <td colSpan={3} className="px-4 py-4 text-xs text-gray-400">
                              (Meals uneditable)
                            </td>
                            <td className="px-6 py-4 text-right space-x-3">
                              <button onClick={() => saveEdit(p.id)} disabled={savingEdit} className="text-green-600 hover:text-green-800 transition">
                                <Check size={18} />
                              </button>
                              <button onClick={cancelEdit} disabled={savingEdit} className="text-gray-400 hover:text-gray-600 transition">
                                <X size={18} />
                              </button>
                            </td>
                          </tr>
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 group">
                          ...
                          <td className="px-4 py-4">{ht ? "✅" : "❌"}</td>
                          <td className="px-4 py-4">{lu ? "✅" : "❌"}</td>
                          <td className="px-4 py-4">{di ? "✅" : "❌"}</td>
